...
import { AddToFavorites } from "@/components/AddToFavorites"; // Importação do novo botão de favoritos

export function Cards({ product }: { product: IProduct }) {
    const { addToCart, alertDialog } = AddToCart();

    return (
        <Card position="relative" _hover={{ boxShadow: "lg", transform: "scale(1.02)", transition: "0.2s" }}>
            <AddToFavorites product={product} /> {/* Botão de Favoritar */}
